# Android Security Digest

Resumen diario automatizado de novedades de seguridad del ecosistema Android.

## Feed RSS

Suscribite al feed para recibir las novedades del día:

**URL del feed:** `https://raw.githubusercontent.com/rrepetto/android-news/claude/digest/feed.xml`

> El feed RSS contiene **solo los ítems del último día** (actualizado en cada corrida diaria). El historial completo está en la carpeta [`digests/`](digests/).

## Historial de digests (últimos 30)

- [2026-09-17](digests/2026-09-17.md)
- [2026-09-16](digests/2026-09-16.md)
- [2026-09-15](digests/2026-09-15.md)
- [2026-09-14](digests/2026-09-14.md)
- [2026-09-13](digests/2026-09-13.md)
- [2026-09-12](digests/2026-09-12.md)
- [2026-09-11](digests/2026-09-11.md)
- [2026-09-10](digests/2026-09-10.md)
- [2026-09-09](digests/2026-09-09.md)
- [2026-09-08](digests/2026-09-08.md)
- [2026-09-07](digests/2026-09-07.md)
- [2026-09-06](digests/2026-09-06.md)
- [2026-09-05](digests/2026-09-05.md)
- [2026-09-04](digests/2026-09-04.md)
- [2026-09-03](digests/2026-09-03.md)
- [2026-09-02](digests/2026-09-02.md)
- [2026-09-01](digests/2026-09-01.md)
- [2026-08-31](digests/2026-08-31.md)
- [2026-08-30](digests/2026-08-30.md)
- [2026-08-29](digests/2026-08-29.md)
- [2026-08-28](digests/2026-08-28.md)
- [2026-08-27](digests/2026-08-27.md)
- [2026-08-26](digests/2026-08-26.md)
- [2026-08-25](digests/2026-08-25.md)
- [2026-08-24](digests/2026-08-24.md)
- [2026-08-23](digests/2026-08-23.md)
- [2026-08-22](digests/2026-08-22.md)
- [2026-08-21](digests/2026-08-21.md)
- [2026-08-20](digests/2026-08-20.md)
- [2026-08-19](digests/2026-08-19.md)

